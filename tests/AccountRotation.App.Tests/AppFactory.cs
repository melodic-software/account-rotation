...
internal sealed class AppFactory : WebApplicationFactory<Program>
{
    public AppFactory()
    {
        Root = Path.Combine(Path.GetTempPath(), "account-rotation-tests", Guid.NewGuid().ToString("N"));
    ...
    public HttpClient CreateMutatingClient()
    {
        ...
        client.DefaultRequestHeaders.Add("X-Account-Rotation", "1");
        return client;
    }

    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.UseSetting("AccountRotation:ConfigPath", ConfigPath);
