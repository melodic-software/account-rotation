...
| A running session writes a stale in-memory `oauthAccount` back to the state file after a switch, so identity and billing disagree | Low (the 2026-09-06 drift probe saw no stale write-back over eight hours with three pre-switch sessions working and the CLI rewriting the file repeatedly) | Medium | The owner record binds the live pair to its account: re-bound when the CLI rotates the token while the state file still names the owner, released when the CLI stamped a later login, kept (so the planner refuses) when the state file carries a block older than the record; the dashboard does not re-patch (corrected 2026-09-06, the earlier row claimed it did) |
